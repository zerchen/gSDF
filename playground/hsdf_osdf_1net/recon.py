...
from net import kinematic_embedding
...
def reconstruct(cfg, filename, hand_sdf_decoder, obj_sdf_decoder, latent_vec, metas, hand_pose_results, obj_pose_results):
    ...
    while head < num_samples:
        ...
        if cfg.hand_branch:
            sdf_hand, _ = decode_sdf(hand_sdf_decoder, latent_vec, hand_sample_subset, 'hand')
    ...
    while head < num_samples:
        ...
        if cfg.hand_branch:
            sdf_hand, predicted_class = decode_sdf(hand_sdf_decoder, latent_vec, hand_sample_subset, 'hand')
    ...
    if cfg.hand_branch:
        ...
        if cfg.output_part_label and vertices is not None and cfg.hand_cls:
            ...
            while head < num_out_vertices:
                ...
                if cfg.hand_encode_style == 'kine':
                    ...
                    hand_sample_subset = sample_subset
                hand_sample_subset = hand_sample_subset.reshape((-1 , cfg.hand_point_latent))
                _, predicted_class = decode_sdf(hand_sdf_decoder, latent_vec, hand_sample_subset, 'hand')
                out_labels[head: min(head + max_batch, num_out_vertices)] = predicted_class.argmax(dim=1).detach().cpu()
                head += max_batch

                if cfg.vis_part_label:
                    write_verts_label_to_obj(vertices, out_labels, os.path.join(cls_sdf_result_dir, ply_filename_hand + "_label.obj"), offset, scale)
                    write_color_labeled_ply(vertices, mesh_faces, out_labels, os.path.join(cls_sdf_result_dir, ply_filename_hand + "_color.ply"), offset, scale)

            write_verts_label_to_npz(vertices, out_labels, os.path.join(cfg.cls_sdf_result_dir, ply_filename_hand + "_label.npz"), offset, scale)

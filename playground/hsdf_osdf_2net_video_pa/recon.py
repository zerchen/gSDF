...
def reconstruct(cfg, filename_frames, model, latent_vec_frames, inputs_frames, metas_frames, hand_pose_results_frames, obj_pose_results_frames):
    model.eval()
    if cfg.hand_branch:
    ...
    num_frames = len(filename_frames)

    latent_vec_refined_frames = [latent_vec_frames[i].unsqueeze(1) for i in range(num_frames)]
    latent_vec_refined_frames = torch.cat(latent_vec_refined_frames, axis=1)
    latent_vec_refined_frames = model.module.feat_transformer(latent_vec_refined_frames)

    for mnt in range(num_frames):
        filename = filename_frames[mnt]
        hand_pose_results = hand_pose_results_frames[mnt]
        obj_pose_results = obj_pose_results_frames[mnt]
        ...
        while head < num_samples:
            ...
            latent_vec_refined, _ = pixel_align(cfg, sample_subset, sample_subset.shape[0], latent_vec_refined_frames[mnt], metas_frames['hand_center_3d'][mnt], metas_frames['cam_intr'][mnt])
            latent_vec_refined = model.module.sdf_encoder(latent_vec_refined)

            if cfg.hand_branch:
                sdf_hand, _ = decode_sdf(hand_sdf_decoder, latent_vec_refined, hand_sample_subset, 'hand')
                samples[head : min(head + max_batch, num_samples), 3] = sdf_hand.squeeze(1).detach().cpu()

            if cfg.obj_branch:
                sdf_obj = decode_sdf(obj_sdf_decoder, latent_vec_refined, obj_sample_subset, 'obj')
        ...
        while head < num_samples:
            ...
            if cfg.hand_branch:
                sdf_hand, predicted_class = decode_sdf(hand_sdf_decoder, latent_vec_refined, hand_sample_subset, 'hand')
